...
plot_synteny_oxford_grid <- function(MBH_table,colors = FALSE,sp1_name = "",sp2_name = "",sp1_keep_chr_names = NULL,sp2_keep_chr_names = NULL,sp1_chr_order = NULL,sp2_chr_order = NULL) {
  ...
  if (colors) {p <- ggplot(MBH_table,aes(x=sp1_index,y=sp2_index,color = sp2_chr)) + geom_jitter(size=0.2,alpha=0.3) }
  else {p <- ggplot(MBH_table,aes(x=sp1_index,y=sp2_index)) + geom_jitter(size=0.2,alpha=0.3)}
  ### build the plot :
  p <- p + theme_tufte() +

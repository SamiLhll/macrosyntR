...
An R package for evaluation of pair-wise synteny conservation at the genome-wide scale. It takes a table of orthologs and genome annotation files formatted as BED to automatically infer significantly conserved blocks, and order them on an oxford grid.
...
| calculate_macrosynt() | compares all the chromosomes to each other and identifies the significantly conserved linkage groups   |
| reorder_synteny()     | takes an mbh_df (from load_mbh_df()) and outputs an mbh_df with chromosome levels reordered by cluster and amount of orthologs (run alone or by setting plot_oxford_grid(...,auto_order_clusters = TRUE) )|
| plot_macrosynt()      | draws a dotplot displaying the significant linkage groups blocks with their relative amount of orthologs      |
| plot_oxford_grid()    | draws an oxford grid from an mbh_df (output of either load_mbh_df() or reorder_synteny()                  |    
...
This package doesn't compute the orthologs. I recommend to compute it as mutual best hits using [mbhxpress](https://github.com/SamiLhll/mbhXpress). It is fast and accurate as it uses diamond blast.   
...
sed -e 's/\"//g' -e 's/,/\t/g' -e 's/chromosome /BFL/g' > Bfloridae.protein_products.bed

 # P.echinospica gff file to bed
fgrep "gene" Pec_genes.ragoo_v1.0.gff | cut -f1,4,5,9 | cut -d ";" -f 1 | \
fgrep "Superscaffold" | sed -e 's/ID=//g' -e 's/Superscaffold/PEC/g' > Pechinospica.protein_products.bed
